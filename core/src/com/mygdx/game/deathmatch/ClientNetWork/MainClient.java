package com.mygdx.game.deathmatch.ClientNetWork;
...
import com.mygdx.game.deathmatch.ClientNetWork.SteckApi.StockRequestService;
...
import com.mygdx.game.deathmatch.VoiceChat.VoiceChatClient;
...
//    }
    ...
    public void sendToServerMyCooedinat() { // сообщить серверу мои координаты
        ...
        Network.PleyerPosition ps = new Network.PleyerPosition();
        updatePingError();
...
/////////////////////////////////////////////////////////////////////Запрос с ответом
        if (object instanceof Network.PleyerPositionNom) {  //получает данные от сервера = если обьект в колекции есть - то обнавляет если нет создает нового
            int nom = ((Network.PleyerPositionNom) object).nom;
            int xp = ((Network.PleyerPositionNom) object).xp;
            int yp = ((Network.PleyerPositionNom) object).yp;
            int rp = ((Network.PleyerPositionNom) object).rot;
...
//////////////////////////////////////////////////////////////////
        if (object instanceof Network.PleyerPosition) {
            int x = ((Network.PleyerPosition) object).xp;
            int y = ((Network.PleyerPosition) object).yp;
            int r = ((Network.PleyerPosition) object).rot;
        }
...
//                System.out.println(ub.names[i].indexOf("||__||"));
        ...
        if (object instanceof Network.rc) { // дисконект пользователя
            try {
                int id = ((Network.rc) object).id;
